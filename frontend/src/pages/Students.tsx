              ...
              {loadingServices ? (
                ...
                services.map((service: any, index: number) => (
                  ...
                  >
                      <div className="mr-4">
                        {(() => {
                          const ICON_MAP: Record<string, { icon: IconType; className: string }> = {
                            FaHome: { icon: FaHome, className: 'text-blue-600' },
                            FaUtensils: { icon: FaUtensils, className: 'text-green-600' },
                            FaHeart: { icon: FaHeart, className: 'text-red-600' },
                            FaGraduationCap: { icon: FaGraduationCap, className: 'text-purple-600' },
                            FaLaptop: { icon: FaLaptop, className: 'text-indigo-600' },
                            FaHandsHelping: { icon: FaHandsHelping, className: 'text-teal-600' },
                            FaBook: { icon: FaBook, className: 'text-amber-600' },
                            FaInfoCircle: { icon: FaInfoCircle, className: 'text-cyan-600' },
                            FaUniversity: { icon: FaUniversity, className: 'text-pink-600' },
                            FaBriefcase: { icon: FaBriefcase, className: 'text-lime-600' }
                          };
                          const config = ICON_MAP[service.icon as keyof typeof ICON_MAP];
                          if (!config) return null;
                          const Icon = config.icon;
                          return <Icon className={`w-8 h-8 ${config.className}`} />;
                        })()}
                      </div>
                  ...
                  <div key={doc.id} className="bg-white border rounded-lg p-6 flex items-center animate-slide-up" style={{ animationDelay: `${800 + index * 100}ms` }}>
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <button
                          onClick={() => window.open(assetUrl(doc.fileUrl), '_blank')}
